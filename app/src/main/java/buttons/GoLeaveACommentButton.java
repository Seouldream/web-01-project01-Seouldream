...
public class GoLeaveACommentButton extends FlatButton {
  ...
  public GoLeaveACommentButton(Account account,Journal journal, List<Comment> comments) {
    this.setText("댓글 보러 가기");
    String registerCommentButtonName = "나도 공감 한마디!";
    this.addActionListener(event -> {
      openCommentsWindow(account,journal, comments,registerCommentButtonName);
    });
  }

  public GoLeaveACommentButton(Account account,Journal journal, List<Comment> comments,String onlyForMe) {
    this.setText("과거의 나가 남긴 한마디 보러가기");
    String registerCommentButtonName = "미래에서 온 나에게 한마디 남기기";
    this.addActionListener(event -> {
      openCommentsWindow(account,journal, comments, registerCommentButtonName,"onlyForMe");
    });
  }

  public void openCommentsWindow(Account account,Journal journal, List<Comment> comments, String registerCommentButtonName) {
  ...
  public void openCommentsWindow(Account account,Journal journal, List<Comment> comments,
                           String registerCommentButtonName,String onlyForMe) {
    writingFrame = new JFrame("오늘의 댓글");
    writingFrame.setSize(500, 600);
    writingFrame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
    writingFrame.setLocationRelativeTo(this);
